...
resource "vsphere_virtual_machine" "bootstrap_ds_cluster" {
  count = "${var.datastore_cluster_id != "" ? (var.bootstrap_complete ? 0 : 1) : 0}"
  depends_on = [
...
resource "vsphere_virtual_machine" "control_plane_ds_cluster" {
  count            = "${var.datastore_cluster_id != "" ? var.control_plane["count"] : 0}"
  depends_on = [
    "module.createignition.module_completed"
  ]
...
resource "vsphere_virtual_machine" "worker_ds_cluster" {
  count            = "${var.datastore_cluster_id != "" ? var.worker["count"] : 0}"
